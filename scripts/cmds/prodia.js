...
module.exports = {
  config: {
    ...
    role: 0,
    shortDescription: {
      en: 'Generate images using prodia API'
    },
    longDescription: {
      en: "Generate images using prodia API"
    },
  ...
  onStart: async function ({ api, event, args, message, usersData }) {
    ...
    message.reply("✅| Creating your Imagination...", async (err, info) => {
      ...
      try {
        const response = await axios.get(`https://king-aryanapis.onrender.com/api/prodia?prompt=${encodeURIComponent(prompt)}&amount=4`);
        api.setMessageReaction("✅", event.messageID, () => {}, true);
